	...
	def app_default(self, c):
		...
		if f.extension is not None:
			if f.extension in ('pdf', ):
				return self.either(c, 'zathura', 'llpp', 'mupdf', 'apvlv', 'evince', 'okular', 'epdfview')
			if f.extension == 'djvu':
				return self.either(c, 'evince')
			if f.extension in ('cbz', 'cbr'):
				return self.either(c, 'evince')
			...
			if f.extension in ('html', 'htm', 'xhtml') or f.extension == 'swf':
				...
				handler = self.either(c,
						'firefox', 'luakit', 'uzbl', 'vimprobable', 'vimprobable2', 'jumanji',
						'seamonkey', 'iceweasel', 'opera', 'surf', 'midori', 'epiphany', 'konqueror')
			...
			if f.extension == 'doc':
				return self.either(c, 'libreoffice', 'abiword', 'soffice', 'ooffice')
			if f.extension in ('odt', 'ods', 'odp', 'odf', 'odg', 'odb', 'sxc', 'stc', 'xls', 'xlsx', 'xlt', 'xlw', 'gnm', 'gnumeric'):
				return self.either(c, 'libreoffice', 'gnumeric', 'kspread', 'soffice', 'ooffice')
		...
		if f.video or f.audio:
			if f.video:
				c.flags += 'd'
			return self.either(c, 'vlc', 'smplayer', 'gmplayer', 'mplayer2',
					'mplayer', 'totem')

		if f.image:
			if c.mode in (11, 12, 13, 14):
				return self.either(c, 'feh', 'set_bg_with_feh')
			else:
